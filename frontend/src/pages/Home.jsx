        ...
        <div className="flex flex-col justify-between">
            ...
            <div className="grid grid-flow-col py-5 auto-cols-mobileTrending overflow-x-auto h-auto mx-3 gap-x-10 md:gap-0 md:py-0 md:grid-cols-5">
              {trendingProducts.map(trendingProduct => (
                <a href={`/product/${trendingProduct._id}`} key={trendingProduct.image} >
                  <div className="flex flex-col p-3 sm:p-5 border-solid border rounded-md border-borderColor md:border-none shadow-[0_0_10px_0_rgba(0,0,0,0.1)] h-full">
                    {/* Product Image */}
          ...
          <div className="bg-backgroundColor flex flex-col justify-between py-5 gap-y-5 md:mt-[3%]"> 
            ...
            <div className="text-center sm:flex sm:justify-evenly sm:items-center">
              ...
              <a href={sessionStorage.getItem("user") ? "/product-listing/category/all" : "/login"}><button className="bg-buttonColor text-white text-base font-bold font-sans rounded-md px-10 py-5" style={{ width: '200px' }}>GRADE NOW</button></a>
            </div>
            {/* Bottom Button */}
            <div className="text-center sm:flex sm:justify-evenly sm:items-center pb-5 sm:pb-0" >
              ...
              <a href="/product-listing/category/all"><button className="bg-buttonColor text-white text-base font-bold font-sans rounded-md px-10 py-5" style={{ width: '200px' }}>FIND NOW</button></a>
            </div>
          </div>
        </div>
          ...
          {recentActivities.map(recent => (
            <a href={`/product/${recent.product._id}`} key={recent.image} >
              <div className="flex flex-col border border-solid border-borderColor rounded-md p-5 gap-y-2 shadow-[0_0_10px_0_rgba(0,0,0,0.1)] min-w-full h-full">
                ...
                {/* Product Image */}
                <img src={getProductImageURL(recent.product.image)} alt="" className="p-3 border border-solid border-borderColor rounded-md h-min w-min mx-auto"/>
                {/* Review's body (Limit:20 characters + '...', which is added after pulling data) */}
                <p>Review:</p>
                <p className="font-light font-sans text-sm">{recent.body.length > 20 ? recent.body.substring(0, 20) + "..." : recent.body}</p>
              </div>
            </a>
          ))}
        </div>
